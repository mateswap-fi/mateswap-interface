...
import { MATE, XMATE } from '../../config/tokens'
...
import { getDayData, useMatePrice } from '../../services/graph'
...
// TODO(ILoveMates): change this
// const fetcher = (query) => request('https://thegraph.mateswap.fi/subgraphs/name/mateswap/bar', query)

export default function Stake() {
  const { i18n } = useLingui()
  const { account, chainId } = useActiveWeb3React()
  const mateBalance = useTokenBalance(account ?? undefined, MATE[chainId])
  const xMateBalance = useTokenBalance(account ?? undefined, XMATE[chainId])

  const matePrice = useMatePrice()

  const { enter, leave } = useSushiBar()

  // TODO(ILoveMates): change this
  // // const { data } = useSWR(`{bar(id: "${XMATE[chainId].address}") {ratio, totalSupply}}`, fetcher)
  ...
  const balance = activeTab === 0 ? mateBalance : xMateBalance
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const apr = (((results[1].volumeUSD * 0.05) / data?.bar?.totalSupply) * 365) / (data?.bar?.ratio * matePrice)
      setApr(apr)
    }
    fetchData()
  }, [data?.bar?.ratio, data?.bar?.totalSupply, matePrice])

  return (
    <Container id="bar-page" className="py-4 md:py-8 lg:py-12" maxWidth="full">
      <Head>
        <title key="title">Stake | MateSwap</title>
        <meta
          key="description"
          name="description"
          content="Stake MATE in return for xMATE, an interest bearing and fungible ERC20 token designed to share revenue generated by all MATE products."
        />
        <meta key="twitter:url" name="twitter:url" content="https://mateswap.fi/stake" />
        <meta key="twitter:title" name="twitter:title" content="STAKE MATE" />
        <meta
        ...
        <meta key="og:title" property="og:title" content="STAKE MATE" />
        <meta key="og:url" property="og:url" content="https://mateswap.fi/stake" />
          ...
          <div className="flex flex-col w-full max-w-xl mt-auto mb-2">
            <div className="flex max-w-lg">
              <div className="self-end mb-3 text-lg font-bold md:text-2xl text-high-emphesis md:mb-7">
                {i18n._(t`Maximize yield by staking MATE for xMATE`)}
              </div>
              {/* <div className="self-start pl-6 pr-3 mb-1 min-w-max md:hidden">
                                <img src={XSushiSignSmall} alt="xMATE sign" />
                            </div> */}
            </div>
            ...
            {/* <div className="flex">
                            ...
                            <div>
                                <StyledLink className="text-sm text-lg whitespace-nowrap md:text-lg md:leading-5">
                                    Tips for using xMATE
                                </StyledLink>
                            </div>
                        </div> */}
          </div>
          <div className="hidden px-8 ml-6 md:block w-72">
            <Image src="/xmate-sign.png" alt="xMATE sign" width="100%" height="100%" layout="responsive" />
          </div>
        </div>
            ...
            {/* <div className="mb-4">
              ...
              <div className="flex items-center justify-between w-full h-24 max-w-xl p-4 rounded md:pl-5 md:pr-7 bg-light-yellow bg-opacity-40">
                <div className="flex flex-col">
                  ...
                  <div className="flex">
                    <a
                      href={`https://analytics.mateswap.fi/bar`}
                      target="_blank"
              ...
              <div className="w-full max-w-xl px-3 pt-2 pb-6 rounded bg-dark-900 md:pb-9 md:pt-4 md:px-8">
                ...
                <div className="flex items-center justify-between w-full mt-6">
                  <p className="font-bold text-large md:text-2xl text-high-emphesis">
                    {activeTab === 0 ? i18n._(t`Stake MATE`) : i18n._(t`Unstake`)}
                  </p>
                  {/* <div className="border-gradient-r-pink-red-light-brown-dark-pink-red border-transparent border-solid border rounded-3xl px-4 md:px-3.5 py-1.5 md:py-0.5 text-high-emphesis text-xs font-medium md:text-base md:font-normal">
                    {`1 xMATE = ${xSushiPerSushi.toFixed(4)} MATE`}
                  </div> */}
                </div>
                ...
                <div className="relative w-full h-0 pointer-events-none bottom-14">
                  ...
                  >
                    <div className="flex space-x-2 ">
                      ...
                      >
                        {`${input ? input : '0'} ${activeTab === 0 ? '' : 'x'}MATE`}
                      </p>
                    </div>
              ...
              <div className="flex flex-wrap">
                <div className="flex flex-col flex-grow md:mb-14">
                  ...
                  <div className="flex items-center space-x-4">
                    <Image
                      ...
                      src="/images/tokens/xmate-square.png"
                      alt="xMATE"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {xMateBalance ? xMateBalance.toSignificant(8) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">xMATE</p>
                    </div>
                  </div>
                  {(xMateBalance && xSushiPerSushi) ?
                    (<div className="mt-3">
                      ~ {xMateBalance.multiply(Math.round(xSushiPerSushi * 1e8)).divide(1e8).toSignificant(8)} MATE
                    </div>) : (<></>)
                  }
                </div>
                ...
                <div className="flex flex-col flex-grow">
                  ...
                  <div className="flex items-center ml-8 space-x-4 md:ml-0">
                    <Image
                      className="max-w-10 md:max-w-16 -ml-1 mr-1 md:mr-2 -mb-1.5 rounded"
                      src="/images/tokens/mate-square.png"
                      alt="MATE"
                      width={64}
                      height={64}
                    />
                    <div className="flex flex-col justify-center">
                      <p className="text-sm font-bold md:text-lg text-high-emphesis">
                        {mateBalance ? mateBalance.toSignificant(8) : '-'}
                      </p>
                      <p className="text-sm md:text-base text-primary">MATE</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col w-full mb-4 mt-7 md:mb-0">
                  {account && (
                    <a
                      href={`https://analytics.mateswap.fi/users/${account}`}
                      target="_blank"
                      rel="noreferrer noopener"
                      className={`
