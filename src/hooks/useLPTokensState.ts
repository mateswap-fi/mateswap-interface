...
const useLPTokensState = () => {
  ...
  const relevantTokenBalances = useTokenBalances(account ?? undefined, YODESWAP_TOKENS[chainId])
  ...
  const updateLPTokens = useCallback(async () => {
    try {
      ...
      for (const [address, obj] of Object.entries(relevantTokenBalances)) {
        ...
        if (yodeswapPairs[chainId] && yodeswapPairs[chainId][address]) {
          console.log('PAIR FOUND')
          data.tokenA = yodeswapPairs[chainId][address].tokenA;
          data.tokenB = yodeswapPairs[chainId][address].tokenB;
        }
